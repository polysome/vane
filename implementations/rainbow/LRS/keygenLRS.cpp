...
void generate_gamma()
{
    gamma=new int[m]; //allocate memory
...
{	
    matrix_B1 = new int *[m] ;//allocate memory
	
	for( int i = 0 ; i < (m) ; i++ )
        matrix_B1[i] = new int[D1];
	
    for(int i=0;i<m;i++){  // define elements
...
{
    matrix_B2 = new int *[o2] ;//allocate memory
	
	for( int i = 0 ; i < (o2) ; i++ )
        matrix_B2[i] = new int[D21];
	
    for(int i=0;i<o2;i++){ //define elements

        matrix_B2[i][0]=mulTable(gamma[i],matrix_B1[i+o1][D1-1]);
