...
int Skein_256_Init(Skein_256_Ctxt_t *ctx, size_t hashBitLen)
{
    ...
    default:
		...
		explicit_bzero(&cfg.w[3], sizeof(cfg) - (3 * sizeof(cfg.w[0]))); /* zero pad config block */

        /* compute the initial chaining values from config block */
		explicit_bzero(ctx->X, sizeof(ctx->X)); /* zero the chaining variables */
...
int Skein_256_InitExt(Skein_256_Ctxt_t *ctx,size_t hashBitLen,uint64_t treeInfo, const uint8_t *key, size_t keyBytes)
{
    ...
    {
		explicit_bzero(ctx->X, sizeof(ctx->X)); /* no key: use all zeroes as key for config block */
    }
    else                                        /* here to pre-process a key */
    {
		...
		explicit_bzero(ctx->X, sizeof(ctx->X)); /* zero the initial chaining variables */
