...
static inline void djb_push_back(djb_t *z, rci_t target, rci_t source, srctyp_t srctyp) {
	...
	if (z->length >= z->allocated) {
		zallocated = z->allocated + M4RI_DJB_BASE_SIZE;
		z->target = (rci_t*)reallocarray(z->target, z->allocated, sizeof(rci_t));
		z->source = (rci_t*)reallocarray(z->source, z->allocated, sizeof(rci_t));
		z->srctyp = (srctyp_t*)reallocarray(z->srctyp, z->allocated, sizeof(srctyp_t));
		z->allocated = zallocated;
	}
